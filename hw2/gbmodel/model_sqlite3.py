...
class model(Model):
    def __init__(self):
        ...
        try:
            ...
            cursor.execute("CREATE TABLE shoplist (name text, street text, city text, state text, zip integer, open_hr text, close_hr text, phone text, website text, drink text, rating integer)")
        cursor.close()
    ...
    def insert(self, name, street, city, state, zip, open_hr, close_hr, phone, website, drink, rating):
        ...
        connection = sqlite3.connect(DB_FILE)
        cursor = connection.cursor()
        cursor.execute("insert into shoplist (name, street, city, state, zip, open_hr, close_hr, phone, website, drink, rating) "
                       "VALUES (:name, :street, :city, :state, :zip, :open_hr, :close_hr, :phone, :website, :drink, :rating)",
                       params)
